...
EXECUTE the following phases SEQUENTIALLY. COMPLETE each phase entirely before proceeding to the next:
    <phase number="1" name="Coding File Change Discovery">
        ...
        <task id="1.4" title="Execute Standardized Commit Workflow">
            EXECUTE and FOLLOW `cat .github/prompts/chains/git-commit.prompt.md` workflow EXACTLY to commit changes with concise yet meaningful message
        </task>
    </phase>
...
<command>**EXECUTE NOW**: Begin autonomous execution of ALL phases following the methodology above. Process EVERY file systematically, address ALL coding standard violations, and commit improvements using the standardized workflow.</command>
